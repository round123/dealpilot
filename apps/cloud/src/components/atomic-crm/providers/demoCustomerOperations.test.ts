...
import { createDemoCustomerOperations } from "./demoCustomerOperations";
import type {
  Company,
  Contact,
  Deal,
...
const setup = () => {
  ...
  return {
    dataProvider,
    operations: createDemoCustomerOperations(dataProvider),
  };
};

describe("demo Customer operations", () => {
  ...
  it("restores deletion-linked reminders after recreating the operations adapter", async () => {
    ...
    const recreatedOperations = createDemoCustomerOperations(dataProvider);
  ...
  it("does not restore a reminder changed after customer deletion", async () => {
    ...
    await createDemoCustomerOperations(dataProvider).restoreCustomer(
      customerId(1),
    );
