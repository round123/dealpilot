...
export function OAuthConsentPage() {
  ...
  useEffect(() => {
    async function loadAuthDetails() {
      if (!authorizationId) {
        setError("crm.auth.oauth.missing_request");
        setLoading(false);
        return;
      }
      if (!authProvider) {
        setError("crm.auth.authentication_unavailable");
        setLoading(false);
        return;
      }
      ...
      if (error) {
        setError(getErrorMessageKey(error, "crm.auth.oauth.load_error"));
      } else {
  ...
  async function handleApprove() {
    ...
    if (error) {
      setError(getErrorMessageKey(error, "crm.auth.oauth.submit_error"));
      setSubmitting(false);
    } else {
    ...
    return (
      <Layout>
        <div className="flex flex-col space-y-2 text-center">
          ...
          <p className="text-destructive">{translate(error)}</p>
        </div>
      </Layout>
    );
  }
      ...
      <Card>
        ...
        <CardFooter className="flex gap-2">
          ...
          <Button
            ...
            className="flex-1"
          >
            {translate("crm.auth.oauth.approve")}
          </Button>
        </CardFooter>
      </Card>
    </Layout>
