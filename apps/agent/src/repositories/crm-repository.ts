...
export async function listCustomers(params: CustomerListQuery) {
  ...
  if (params.cursor) {
    const [cursorRow] = await db.select().from(customers)
      .where(eq(customers.id, params.cursor)).limit(1);
    if (cursorRow) {
      const cursorValue = params.sort === "name" ? cursorRow.name
        : params.sort === "grade" ? cursorRow.grade
        : params.sort === "updated_at" ? cursorRow.updated_at
        : cursorRow.created_at;
      conditions.push(sql`(${sortColumn} > ${cursorValue} OR (${sortColumn} = ${cursorValue} AND ${customers.id} > ${params.cursor}))`);
    }
  }
...
export async function getCustomerDetail(customerId: string) {
  ...
  const [customerContacts, accounts, customerProjects, recentFollowUps, openReminders] = await Promise.all([
    db.select().from(contacts).where(eq(contacts.customer_id, customerId)),
    db.select().from(social_accounts).where(eq(social_accounts.customer_id, customerId)),
    db.select().from(projects).where(eq(projects.customer_id, customerId)),
    db.select().from(follow_ups).where(eq(follow_ups.customer_id, customerId))
      .orderBy(desc(follow_ups.occurred_at)).limit(10),
...
export async function updateCustomerRecord(customerId: string, input: CustomerUpdate) {
...
export async function softDeleteCustomerRecord(customerId: string) {
  return db.transaction(async (tx) => {
    ...
    const now = new Date().toISOString();
    const openReminders = await tx.select({
      id: reminders.id,
      status: reminders.status,
      resolution: reminders.resolution,
    }).from(reminders).where(and(
      eq(reminders.customer_id, customerId),
      sql`${reminders.status} IN ('pending', 'snoozed', 'overdue')`,
    ));

    await tx.update(customers).set({ deleted_at: now, updated_at: now })
...
export async function restoreCustomerRecord(customerId: string) {
  return db.transaction(async (tx) => {
    ...
    const [deletionEvent] = await tx.select({ metadata: local_events.metadata })
      .from(local_events)
      .where(and(
    ...
    const [restored] = await tx.update(customers)
...
function parseDeletedReminderStates(metadata: string | null | undefined): RestorableReminderState[] {
  if (!metadata) return [];
  try {
    const parsed = JSON.parse(metadata) as { reminders?: unknown };
    if (!Array.isArray(parsed.reminders)) return [];
    return parsed.reminders.filter((value): value is RestorableReminderState => {
      if (!value || typeof value !== "object") return false;
      const reminder = value as Partial<RestorableReminderState>;
      return typeof reminder.id === "string"
        && (reminder.status === "pending" || reminder.status === "snoozed" || reminder.status === "overdue")
        && (reminder.resolution === null || typeof reminder.resolution === "string");
    });
  } catch {
    return [];
  }
...
export async function mergeCustomerRecords(
  ...
  return db.transaction(async (tx) => {
    const [source] = await tx.select().from(customers)
      .where(eq(customers.id, sourceId)).limit(1);
    const [target] = await tx.select().from(customers)
      .where(eq(customers.id, targetId)).limit(1);
    if (!source) return { status: "source_not_found" as const };
    if (!target) return { status: "target_not_found" as const };

    await tx.update(contacts).set({ customer_id: targetId })
      .where(eq(contacts.customer_id, sourceId));
    await tx.update(social_accounts).set({ customer_id: targetId })
      .where(eq(social_accounts.customer_id, sourceId));
    await tx.update(projects).set({ customer_id: targetId })
      .where(eq(projects.customer_id, sourceId));
    await tx.update(follow_ups).set({ customer_id: targetId })
      .where(eq(follow_ups.customer_id, sourceId));
    await tx.update(reminders).set({ customer_id: targetId })
      .where(eq(reminders.customer_id, sourceId));

    const now = new Date().toISOString();
    if (Object.keys(targetUpdates).length > 0) {
      await tx.update(customers).set({ ...targetUpdates, updated_at: now })
...
export async function updateContactRecord(contactId: string, input: ContactUpdate) {
  const [updated] = await db.update(contacts).set(input)
...
export async function listSocialAccounts(customerId: string) {
  return db.select().from(social_accounts)
    .where(eq(social_accounts.customer_id, customerId));
}

export async function findSocialAccount(platform: string, normalizedIdentifier: string) {
  const [account] = await db.select().from(social_accounts).where(and(
...
export async function insertSocialAccount(
  ...
  const [created] = await db.insert(social_accounts).values({
    customer_id: customerId,
...
export async function findMatchingCustomers(platform: string, normalizedIdentifier: string) {
  return db.select({ customer: customers }).from(social_accounts)
...
export async function upsertManualBinding(
  ...
  if (existing) {
    const [updated] = await db.update(social_accounts).set({
      customer_id: customerId,
      raw_identifier: rawIdentifier,
      manually_bound: true,
    }).where(eq(social_accounts.id, existing.id)).returning();
    return updated;
  }
...
export async function deleteBinding(platform: string, normalizedIdentifier: string) {
  await db.delete(social_accounts).where(and(
    eq(social_accounts.platform, platform),
    eq(social_accounts.normalized_identifier, normalizedIdentifier),
  ));
}
...
export async function listFollowUps(params: FollowUpListQuery) {
  const conditions = [];
  if (params.customer_id) conditions.push(eq(follow_ups.customer_id, params.customer_id));
  if (params.project_id) conditions.push(eq(follow_ups.project_id, params.project_id));
  if (params.cursor) {
    const [cursorRow] = await db.select().from(follow_ups)
      .where(eq(follow_ups.id, params.cursor)).limit(1);
    if (cursorRow) {
      conditions.push(sql`(${follow_ups.occurred_at} < ${cursorRow.occurred_at} OR (${follow_ups.occurred_at} = ${cursorRow.occurred_at} AND ${follow_ups.id} < ${params.cursor}))`);
    }
  }

  const base = db.select().from(follow_ups)
...
export async function insertFollowUp(input: FollowUpCreate) {
  const [created] = await db.insert(follow_ups).values({
...
export async function updateFollowUpRecord(followUpId: string, input: FollowUpUpdate) {
  const [updated] = await db.update(follow_ups).set(input)
    .where(eq(follow_ups.id, followUpId)).returning();
  return updated;
}
