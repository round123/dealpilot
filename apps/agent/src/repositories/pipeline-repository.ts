...
export async function listProjectRecords(query: ProjectListQuery) {
  const conditions = [];
  if (query.customer_id) conditions.push(eq(projects.customer_id, query.customer_id));
  if (query.stage) conditions.push(eq(projects.stage, query.stage));
  if (query.grade) conditions.push(eq(projects.grade, query.grade));
  if (query.cursor) {
    const [cursor] = await db.select().from(projects)
      .where(eq(projects.id, query.cursor)).limit(1);
    if (cursor) {
      conditions.push(sql`(${projects.created_at} > ${cursor.created_at} OR (${projects.created_at} = ${cursor.created_at} AND ${projects.id} > ${query.cursor}))`);
    }
  }
...
export async function updateProjectRecord(projectId: string, input: ProjectUpdate) {
...
export async function updateProjectStageRecord(projectId: string, stage: string) {
...
export async function updateRiskRecord(riskId: string, input: RiskUpdate) {
  ...
  const [updated] = await db.update(risks).set({
    ...input,
    ...(input.status ? { handled_at: handled ? new Date().toISOString() : null } : {}),
  }).where(eq(risks.id, riskId)).returning();
  return updated;
}

export async function insertMilestone(projectId: string, input: MilestoneCreate) {
...
export async function updateMilestoneRecord(milestoneId: string, completed: boolean) {
  const [updated] = await db.update(milestones).set({ completed })
    .where(eq(milestones.id, milestoneId)).returning();
  return updated;
}

export function findUpcomingMilestones(fromDate: string, toDate: string) {
  return db.select({ milestone: milestones, project: projects })
    .from(milestones)
...
export async function hasMilestoneReminder(projectId: string, milestoneId: string) {
