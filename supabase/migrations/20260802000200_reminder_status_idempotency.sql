-- All reminder state transitions use one owner-scoped, replayable transaction.

create function public.update_reminder_status_idempotent(
...
begin
  ...
  if current_reminder.status not in ('pending', 'snoozed', 'overdue') then
...
-- State changes must not bypass conflict detection or the replay ledger.
revoke update (
  status, snooze_until, resolution
) on table public.reminders from authenticated;
