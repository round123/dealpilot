...
export const customers = sqliteTable(
  ...
  (table) => [
    index("idx_grade").on(table.grade),
...
export const contacts = sqliteTable(
  ...
  (table) => [
    index("idx_customer_id").on(table.customer_id),
...
export const social_accounts = sqliteTable(
  ...
  (table) => [
    uniqueIndex("idx_platform_normalized").on(table.platform, table.normalized_identifier),
    index("idx_customer_id").on(table.customer_id),
  ]
);
...
export const follow_ups = sqliteTable(
  ...
  (table) => [
    index("idx_customer_id").on(table.customer_id),
    index("idx_project_id").on(table.project_id),
    index("idx_occurred_at").on(table.occurred_at),
  ]
);
...
export const reminders = sqliteTable(
  ...
  (table) => [
    index("idx_status").on(table.status),
    index("idx_due_at").on(table.due_at),
    index("idx_customer_id").on(table.customer_id),
    index("idx_project_id").on(table.project_id),
