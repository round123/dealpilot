# 外贸经理个人工作台 系统架构设计 V1.3

> 本文档定义 DealPilot V1 的本地优先技术架构，承接《外贸经理个人工作台 PRD V1.5》。业务功能与产品验收以 PRD 为准，本文只描述系统边界、组件职责、数据存储、通信、安全、部署及演进方案。
...
| 状态 | 架构评审稿 |
| 日期 | 2026-07-28 |
| 关联需求 | [外贸经理个人工作台_PRD_V1.5.md](./外贸经理个人工作台_PRD_V1.5.md) |
| 一期平台 | Windows 10/11、Chrome / Edge 最新两个版本 |
| 核心约束 | 单人使用、无需登录、业务数据只存本机、无云端业务数据库 |
| 一期 AI | 不实现；V2 AI 走独立 Python 服务，经 HTTP 接入 AI Gateway |
| 交付形态 | 单个 `DealPilot-Setup.exe` 安装包；启动 `dealpilot-agent.exe` 后自动打开浏览器到本地工作台 URL |

### 1.1 本次修订（V1.3）

V1.3 将本地 Agent 运行时和打包方案从 Node.js + Node SEA 切换为 **Bun**。这是基于 2025–2026 生态调研后的决策：Node SEA 到 Node 25.5+ 才简化构建命令，ESM 模式下不支持 V8 代码缓存和启动快照；而 Bun `build --compile` 已成熟，支持全栈打包（前端资源 + 后端 + SQLite 进单 exe），启动更快，体积更小，Hono 在 Bun 上原生运行。

本次修订要点：

1. **运行时从 Node.js 改为 Bun 1.3+**：Hono 在 Bun 上原生运行（无需 Fastify 备选）；`bun:sqlite` 是 Bun 内置的零编译 SQLite 驱动；`Bun.password.hash` 内置 Argon2id，不需要 `@node-rs/argon2`
2. **打包从 Node SEA 改为 `bun build --compile`**：Bun 支持将 HTML/React 前端 + 后端 API + 依赖全部打进单个 `dealpilot-agent.exe`，不再需要静态资源和 Agent 分别打包再版本同步
3. **SQLCipher 路径需验证**：`bun:sqlite` 是否支持 SQLCipher 扩展、`better-sqlite3` 在 Bun 运行时下的兼容性——这两条是 G1 Spike S6 的核心验证项
4. **A-09 已决策**：选 Bun，不再作为待决策项
5. **安装包体积预期进一步缩小**：Bun 全栈打包 + 内置 SQLite，预估 25–40MB（V1.2 预估 30–50MB）
6. **插件框架从 Plasmo 改为 WXT**：统一 Vite 工具链——前端用 Vite 构建，插件用 WXT（Vite-native），共享构建心智模型和 HMR 体验；WXT 维护更活跃、跨浏览器支持更强、框架无关（React 一等公民）
...
V1 采用"浏览器工作台 + 浏览器插件 + 本地 Agent + 本地数据库"的四层架构：

- `dealpilot-agent.exe`（Bun `build --compile` 单文件）作为本地 Agent，同时承担：HTTP 业务 API、Web 工作台静态资源托管、系统托盘、系统通知、Native Messaging Host
- 浏览器工作台由用户浏览器加载，访问 `http://127.0.0.1:port/`，调用 Agent 业务 API
- Chrome / Edge 插件负责 WhatsApp / Telegram 页面集成、浮窗和 popup
- SQLite 是业务数据的唯一事实源，不建设云端业务数据库
- 用户双击 `dealpilot-agent.exe` 启动后，Agent 自动打开默认浏览器到工作台 URL；插件通过 Native Messaging 完成 Agent 发现和可信引导；两端均通过 `127.0.0.1` REST API 调用本地后端
- V2 AI 作为独立 Python 服务接入，通过 HTTP 调用 AI Gateway；本地模型走 Ollama HTTP，在线模型走厂商 API，嵌入式本地推理需要时再单独评估

这仍然属于本地优先架构：业务后端在本机运行，业务数据的权威副本也只在本机。AI 服务在 V2 才部署，且与业务后端进程隔离，互不影响。

## 3. 目标与非目标

### 3.1 架构目标

1. 客户、项目、跟进和提醒数据默认不离开用户设备
2. 插件浮窗、popup 和工作台共用同一数据源，避免多份数据同步
3. 产品不依赖登录和云服务即可完成一期核心流程
4. 浏览器关闭时，本地 Agent 仍可处理到期任务并发出系统通知
5. 支持 Excel 导入、全量备份、恢复和应用升级迁移
6. 模块边界能够承接后续客户背调、推进建议和销售教练能力
7. 用户只需运行一个安装程序，并可从桌面快捷方式一键启动完整工作台
...
| 编号 | 决策 | 理由 |
|------|------|------|
| ADR-01 | 本地 Agent 是业务后端 | 提醒、Excel、备份等业务能力不受浏览器沙箱限制 |
| ADR-02 | SQLite 是唯一业务数据源 | 单用户、本地部署、事务和备份需求与 SQLite 高度匹配 |
| ADR-03 | 插件不保存完整业务数据 | 避免 IndexedDB 与后端数据库形成双写和同步问题 |
| ADR-04 | 工作台 UI 由浏览器加载 | 由 Node Agent 托管静态资源；避免原生桌面壳的体积、工具链和桥接开销 |
| ADR-05 | Native Messaging 只做引导 | 用于发现、启动和认证本地 Agent；常规业务走 REST API |
| ADR-06 | 后端只监听回环地址 | 防止同一局域网中的其他设备访问本地数据 |
| ADR-07 | 提醒状态落库 | 进程重启、系统休眠或浏览器关闭后仍能恢复任务 |
| ADR-08 | AI 通过适配器接入 | 核心业务不依赖具体模型、向量库或供应商；AI 作为独立 Python 服务经 HTTP 调用 |
| ADR-09 | 无远程埋点 | 指标本地计算，用户主动导出匿名统计报告 |
| ADR-10 | 单个 EXE 安装包 | NSIS 安装包携带 Bun `build --compile` 单文件、静态资源、SQLCipher 和 Native Messaging manifest |
| ADR-11 | 扩展采用商店确认安装 | 个人电脑遵守浏览器安全策略；企业受管设备才支持策略静默安装 |
| ADR-12 | 业务后端语言与 AI 服务语言解耦 | V1 业务后端选最适合 V1 实际能力的语言；V2 AI 真要 Python 时作为独立服务部署，业务后端零返工 |
| ADR-13 | 不使用原生桌面壳（Tauri / Electron） | V1 业务能力全部在 Bun Agent；原生壳仅提供 WebView、托盘和通知，可以用浏览器加载 UI + Bun 库替代，避免 Rust 工具链和跨语言桥接复杂度 |
