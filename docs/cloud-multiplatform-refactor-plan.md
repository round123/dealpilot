...
> 状态：Web/Cloud V2.1 已定稿；Supabase 开发项目持续部署验收中
> 日期：2026-07-31
> 目标版本：V2
...
以上历史证据不能替代 Supabase 开发项目的事务、用户隔离、Auth、Storage、RPC 和迁移回滚验收。双用户 browser、REST、RPC、Edge、复合外键和 Storage runner 已实现并串入 CI 定义，但尚未在开发项目或远端 CI 执行，因此 P3 仍未通过。
