...
- 使用 Supabase Auth 管理注册、邮箱验证、登录、密码重置、会话和账号删除。
...
| P6 云端发布 | Auth 邮件流程、隐私、账号删除、监控、灰度、备份恢复和应用回滚 | canary/production 发布门与演练证据完整 | 3-4 天 |
...
| P6 | 待验收 | preview/canary/production workflow、向前兼容 migration 检查和旧应用回滚流程 | 真实 canary、生产 smoke、账号删除与合规演练 |
...
6. Customer 软删除保留 30 天；账号删除和备份保留遵循发布前确认的隐私与删除 SLA。
...
- 注册、验证、登录、密码重置、会话和账号删除流程通过真实环境验收。
