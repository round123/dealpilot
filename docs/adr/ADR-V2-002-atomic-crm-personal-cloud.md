...
日常开发直接连接受控的 Supabase 开发项目，本机只运行 Web/PWA 的 Vite 服务。CI 可使用临时本地 Supabase 从空库执行 PostgreSQL migration、RLS、事务函数、Storage 和 API 契约测试，但开发者电脑不要求 Docker、Supabase CLI 或第二套业务数据库。当前 demo 不读取或迁移 SQLite 数据。
...
8. Customer 合并、软删除、恢复、提醒状态联动、CSV/XLSX 导入提交和云备份恢复必须在数据库事务内完成。
9. 删除现有 Agent、托盘、系统通知、Native Messaging、EXE 和 NSIS 构建链；不把 SQLite 迁移工具或本地数据入口作为产品能力。Web/PWA 是唯一主界面。
10. PostgreSQL 从账号创建开始永久成为唯一事实源，不建设本地写模式、双写或回写 SQLite 的链路。
...
- Atomic 与 DealPilot 业务模型不同，需要明确字段映射和行为验收。
- Supabase/PostgREST 与原计划 OpenAPI 包络不同，需要自有客户端适配层稳定调用语义。
- 上游升级可能与 DealPilot 的 schema、认证和业务事务改造冲突。
- 日常开发依赖受控 Supabase 开发项目；CI 的临时 Supabase 只用于自动化门禁。V2 Web/PWA 不包含 Agent、托盘、系统通知、Native Messaging 或 SQLite 数据迁移入口。
...
- **废弃当前仓库直接在 Atomic 仓库重写**：会丢失扩展、领域逻辑和行为测试，并放大切换风险。
